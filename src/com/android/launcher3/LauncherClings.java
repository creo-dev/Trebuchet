...
class LauncherClings implements OnClickListener {
    ...
    private boolean clingDone = false;
    ...
    public void showLongPressCling(boolean showWelcome) {
        ViewGroup root = (ViewGroup) mLauncher.findViewById(R.id.launcher);
        ClingView cling = (ClingView)mInflater.inflate(R.layout.longpress_cling, root, false);
        ...
        clingView = cling;
        mLauncher.getQsbBar().setVisibility(View.GONE);
        ...
        View clingTextView = content.findViewById(R.id.cling_dismiss_longpress_info);
        cling.bindLauncherandTextView(mLauncher, clingTextView, this);
        clingTextView.setOnTouchListener(new View.OnTouchListener() {
            @Override
        ...
        if (TAG_CROP_TOP_AND_SIDES.equals(content.getTag())) {
            Drawable bg = new BorderCropDrawable(mLauncher.getResources().getDrawable(R.drawable.creo_final_cling_bg),
    ...
    public void dismissLongPressCling() {
        Runnable dismissCb = new Runnable() {
            public void run() {
        ...
        mLauncher.getWorkspace().post(dismissCb);
        if(mLauncher.getQsbBar() != null && mLauncher.isSearchBarEnabled()){
            mLauncher.getQsbBar().setVisibility(View.VISIBLE);
        }
        clingDone = true;
    }

    public boolean isClingDone() {
        return clingDone;
    }

    public void setClingDone(boolean clingDone) {
        this.clingDone = clingDone;
    }
    ...
    public boolean areClingsEnabled() {
        if (DISABLE_CLINGS) {
            return false;
        }
